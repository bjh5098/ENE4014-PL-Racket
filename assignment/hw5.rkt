...
(define (eval-under-env e env)
  (cond [(var? e) 
        ...
        ;; CHANGE add more cases here ;; 2015004575 JunHyeok Bae
        ...
        [(fun? e)
         (letrec ([funname (fun-nameopt e)]
                  [funarg (fun-formal e)]
                  [newenv env] ;;to enviroment
                  [clos (closure newenv e)]
                  )
           clos)]
        ...
        [(mlet? e)
         (letrec ([v (eval-under-env (mlet-e e) env)]
                  [newenv (cons (cons (mlet-var e) v) env)]);;var-string(mlet-var e)????No
        ...
        [(isaunit? e)
          (letrec(
                  [v (eval-under-env (isaunit-e e) env)])
            (if (aunit? v)
                (int 1)
                (int 0)))]
        [(closure? e) e]
...
(define mupl-map 
  (fun #f "func" ;;function on list
       (fun "op" "lst" ;;traverse list
            (ifaunit (var "lst")
                (aunit)
                (apair (call (var "func") (fst (var "lst")))
                       (call (var "op") (snd (var "lst"))))))))
